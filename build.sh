...
export OPENMSX_APP_PATH=../openMSX_night_build/bin/
...
export PEEP_FILE=$CURRENT_DIR/fusion-c/lib/peep.def
...
#      fi
      #exec -c `${OPENMSX_APP_PATH}openmsx -script ${MSX_MACHINE_SCRIPT_PATH}../emul_start_config.txt ${CARTRIDGE_SLOT_A}` & disown
      ${OPENMSX_APP_PATH}openmsx -script ${MSX_MACHINE_SCRIPT_PATH}../emul_start_config.txt ${CARTRIDGE_SLOT_A}
      popd > /dev/null 2>&1
